...
**Goal:** Add TV show indexing, browsing, and playback with an episode browser
alongside the existing movie support.

**Architecture:** Three new database tables (shows, seasons, episodes) with a
parallel show scanner service. TMDB TV endpoints for metadata. New API routes
for shows/episodes. Frontend gets a shows page, show detail page, and an episode
side panel in the player. Home page merges both content types.

**Tech Stack:** Bun, Hono, Drizzle ORM/SQLite, React 19, React Router, TanStack
Query, video.js, Tailwind CSS v4, Lucide icons.
...
Run: `bunx vitest run src/services/episode-parser.test.ts` Expected: FAIL —
module not found
...
Run: `bunx vitest run src/services/episode-parser.test.ts` Expected: All tests
PASS
...
Run: `bunx vitest run src/services/show-scanner.test.ts` Expected: FAIL — module
not found
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      if (seasonNumber !== null) {
        ...
        const nestedEpisodes = await scanShowFolder(
...
export async function discoverShows(
  ...
  return Array.from(showMap.values()).filter((show) => show.episodes.length > 0)
}
```

- [ ] **Step 4: Run tests to verify they pass**

Run: `bunx vitest run src/services/show-scanner.test.ts` Expected: All tests
PASS
...
Add the following to `src/database/schema.ts` after the existing `watchProgress`
table and before the type exports:
...
Update the `watchProgress` table: change `movieId` from `.notNull()` to
nullable, and add `episodeId`:
...
Add the following after the existing `watch_progress` CREATE TABLE in
...
try {
  sqlite.exec(
    'ALTER TABLE watch_progress ADD COLUMN episode_id INTEGER REFERENCES episodes(id)'
  )
} catch {
...
Run: `bunx tsc --noEmit` Expected: No type errors
...
Add these interfaces after the existing `MovieMetadata` interface in
...
Add these functions at the end of `src/services/tmdb.ts` (before the closing of
the file):
...
Add the following imports and function at the end of
`src/services/show-scanner.ts`:
...
export async function scanShowLibrary(libraryPath: string): Promise<{
  ...
  for (const discovered of discoveredShows) {
    console.log(
      `  Found show: ${discovered.name} (${discovered.episodes.length} episodes)`
    )
...
Run: `bun run src/scripts/scan.ts --shows "D:\Downloads\TV Shows"` Expected:
Shows are discovered, grouped (Silicon Valley seasons merged, First Wave seasons
merged), TMDB metadata fetched, episodes persisted to database. Output shows
show names and episode counts.
...
In `src/api/routes/progress.ts`, update the `continue-watching` endpoint.
Replace the entire route handler for `/continue-watching` with:
...
Run: `curl http://localhost:3000/api/library/shows | head -c 500` Expected: JSON
array of shows with seasonCount and episodeCount

Run: `curl http://localhost:3000/api/library/shows/1 | head -c 500` Expected:
JSON show object with nested seasons and episodes arrays
...
Run: `bunx tsc --noEmit` Expected: Type errors in components that use old
ContinueWatchingItem type. These will be fixed in the next tasks.
...
Run: `bunx tsc --noEmit` Expected: Type errors in Home.tsx and Movies.tsx since
they still reference old GenreRow shape. Will fix in next tasks.
...
The `MovieRow` component is no longer used by Home.tsx or Movies.tsx. Check if
it's imported elsewhere. If not, leave it in place — it may be useful for the
shows page or other contexts. No changes needed.

- [ ] **Step 4: Run typecheck**

Run: `bunx tsc --noEmit` Expected: No type errors (or minor errors from pages
not yet created)
...
- Clicking a show card links to `/shows/:id` (will 404 for now — that's
  expected)
...
In `src/app/components/PageHeader.tsx`, add a "TV Shows" nav link after the
"Movies" NavLink:
...
In `src/app/components/VideoControls.tsx`, update the interface and add the
episodes button:
...
Add the episodes button in the controls bar. In the right-side div (before
fullscreen), add:
...
Run: `bunx vitest run` Expected: All tests pass
...
Run: `bunx eslint src/` Expected: No lint errors
...
8. Continue watching: Play a movie partway and an episode partway, return to
   home, verify both appear in continue watching row
