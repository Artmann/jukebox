...
/**
 * Owns the video.js player lifecycle: one instance is created on mount and
 * disposed on unmount, while source, start-position, and poster changes are
 * applied to that same instance so consumers never see a disposed player.
 * Render the player into the returned `videoRef` container.
 */
export function useVideoJsPlayer({
  ...
  src,
  startSeconds
}: VideoJsPlayerOptions): VideoJsPlayerResult {
  ...
  const notifyReady = useEffectEvent((player: Player) => {
    onReady?.(player)
  })

  const notifySourceResolved = useEffectEvent((resolved: ResolvedSource) => {
